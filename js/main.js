...
$(function () {
    ...
    $(document).on('click', '#form-send', function () {
        ...
        try {
            ...
            // Gather headers
            var headersList = strToHeaders($formHeaders.val());
            var headers = {};
            for (var i in headersList) {
                if (headersList[i].name) {
                    headers[headersList[i].name] = headersList[i].value;
                }
            }
            ...
            $formBody2Image.html('');
            $formLabelBody2.attr('for', 'form-body2').text('Answer body:');
            ...
            if (['GET', 'HEAD'].indexOf(method.toUpperCase()) === -1 && body) {
            ...
            fetch(url, fetchOptions).then(async function (response) {
                ...
                var displayValue = response.status + (response.statusText ? ' - ' + response.statusText : '');
                $('#form-status')
                    .val(displayValue)
                    .removeClass('blink')
                    .removeClass('ok')
                    .removeClass('error')
                    .addClass((response.status >= 200 && response.status < 300) ? 'ok' : 'error');

                $('.hint').show();
                $('#hint').html(getStatusHint(response.status));
                $('#form-time').val(time + ' ms');

                $formHeaders2.val(headersToStr(resHeaders));
                autosize.update($formHeaders2);
                $formBody2.val(format(text, mimeType));
                autosize.update($formBody2);
